...
class Gui:
    ...
    def __packing(self):
        ...
        self.__score_label.pack(side=LEFT, padx=MED_P, pady=MED_P)
        self.__best_score_label.pack(side=LEFT, pady=MED_P)
        self.__num_of_rounds.pack(side=LEFT, padx=MED_P, pady=MED_P)
        self.__replay_button.pack(side=LEFT, padx=MED_P, pady=BIG_P)
        self.__exit_button.pack(side=LEFT, padx=MED_P, pady=BIG_P)
        self.__undo_button.pack(side=LEFT, padx=MED_P, pady=BIG_P)
    ...
    def __display_cells(self, flag):
        """
        update the board cubes with the letters given in the random board.
        :return: None
        """
    ...
    def win(self):
        ...
        win_msg_label = Label(win_msg, bg=POP_UP_MSG_COLOR, text=YOU_WON,
                              font=(LETTERS_FONT, LETTERS_SIZE + 5, BOLD))
        ...
        exit_button = Button(buttons_label, text=NO, bg=POP_UP_BUTTON_COLOR,
                             ...
                             command=self.__window.destroy, width=3, height=1)
        keep_play_button = Button(buttons_label, text=YES,
                                  bg=POP_UP_BUTTON_COLOR,
                                  font=(LETTERS_FONT, LETTERS_SIZE, BOLD),
                                  command=win_msg.destroy, width=3, height=1)
        win_msg_label.pack()
        play_again_q.pack()
        buttons_label.pack(pady=BIG_P, padx=BIG_P)
        keep_play_button.pack(side=LEFT, pady=BIG_P, padx=BIG_P)
        exit_button.pack(side=LEFT, pady=BIG_P, padx=BIG_P)
        self.__win = True

    def _replay(self):
        self.__game_board.replay()
        self.__win = False
        self.__display_cells(False)
        self.__score_label[TEXT] = SCORE + str(self.__game_board.get_score())
        self.__num_of_rounds[TEXT] = NUM_OF_ROUNDS + str(
